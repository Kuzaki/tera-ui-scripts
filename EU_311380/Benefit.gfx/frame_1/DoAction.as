...
{
    gs.TweenLite.killTweensOf(eventMc.btn.effect,false);
    gs.TweenLite.to(eventMc.btn.effect,0.5,{_alpha:num,onComplete:effectCheck});
}
function effectCheck()
{
    if(effectMode)
    {
        if(eventMc.btn.effect._alpha > 50)
        {
            newEffect(0);
        }
        else
...
{
    lib.Debuger.trace("ToGame_Benefit_PcEventClick");

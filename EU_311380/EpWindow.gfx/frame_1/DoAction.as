...
myListener.OnGame_EpWindow_SetInfo = function(exp, nextEp, ep, lvNum, gradeNum)
{
    ...
    var _loc6_ = lib.util.UIString.getUIString("$670014","count",String(ICON_SRC + FONT_COLOR_EP + nextEp + "</font>"));
    container_mc.expTxt.htmlText = _loc6_;
    container_mc.persentTxt.text = _loc2_ + "%";
    container_mc.gageMc._width = _loc3_ * GAGE_WID;
    container_mc.totalMc.mc.txt.htmlText = EP_STR0 + " " + ICON_SRC + FONT_COLOR_EP + _loc1_[2] + "</font>";
    useEp = _loc1_[0];
    saveEp = Number(_loc1_[1]);
    container_mc.gradeMc.gotoAndStop(Number(gradeNum) + 1);
    container_mc.epPoint.mc.txt.htmlText = FONT_COLOR_EP + ICON_SRC1 + saveEp + "</font>";
    var _loc5_ = lib.util.UIString.getUIString("$670008","count",_loc2_);
    lib.manager.ToolTip.add(container_mc.nextHit,_loc5_,1);
};
...
myListener.OnGame_EpWindow_SetList = function(_type, _data)
{
    ...
    {
        if(container_mc.checkMc.checkBtn.checked)
        {
            container_mc.checkMc.gotoAndPlay(2);
        }
        else
        {
            container_mc.checkMc.gotoAndStop(1);
        }
    ...
    if(container_mc.checkMc.checkBtn.checked)
    {
        var _loc9_ = container_mc["tag" + focusTab];
        if(_loc9_)
        {
            container_mc.checkMc.gotoAndPlay(2);
        }
...
myListener.OnGame_EpWindow_SetFocusInfo = function(_data, _connect)
{
    ...
    _loc1_._visible = true;
    container_mc.UIDrager._x = -470;
    container_mc.UIDrager._width = 1570;
    container_mc.UIDrager.setData(UIName,UI);
    var _loc3_ = pointFocus.lockMc._currentframe;
    var _loc6_ = pointFocus.typeMc._currentframe;
    var _loc7_ = pointFocus.colorMc._currentframe;
    var _loc4_ = pointFocus.numMc._currentframe;
    _loc1_.lockMc.gotoAndStop(_loc3_);
    _loc1_.typeMc.gotoAndStop(_loc6_);
    _loc1_.colorMc.gotoAndStop(_loc7_);
    SetConnect(1);
    connectData = [];
    if(_loc3_ == 2)
    {
...
{
    container_mc.UIDrager._x = 0;
    container_mc.UIDrager._width = 1100;
    container_mc.UIDrager.setData(UIName,UI);

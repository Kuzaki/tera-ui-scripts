...
function closeEvent()
{
    myListener.OnGame_GageBoss_ClearBuff();
    gs.TweenLite.killTweensOf(beforeFocus,false);
    ...
    gs.TweenLite.killTweensOf(gageFocus.effectBar,false);
    gs.TweenLite.killTweensOf(gageFocus.effectGage,false);
    gs.TweenLite.killTweensOf(gageFocus.currentGage,false);
}
function StackMotionEnd()
{
    gs.TweenLite.to(container_mc.stackGage,0.2,{_alpha:100});
}
...
function ToGame_GageBoss_PercentVisible(type)
{
    getURL("FSCommand:ToGame_GageBoss_PercentVisible",type);
}
...
var gageArr = [];
var gageFocus = container_mc.gageBox.gageMc0;
var gageTop = container_mc.gageBox.gageMc0;
...
container_mc.UIDrager.onRelease2 = function()
{
    if(container_mc.UIDrager.pos_state == "false")
    {
        ...
        {
            myListener.OnGame_GageBoss_PercentVisible(0);
            ToGame_GageBoss_PercentVisible(0);
        }
        else
        {
            myListener.OnGame_GageBoss_PercentVisible(1);
            ToGame_GageBoss_PercentVisible(1);
        }
    }
};
...
myListener.OnGame_GageBoss_Add = function(index)
{
    var _loc1_ = container_mc.gageBox["gageMc" + index];
    _loc1_._visible = true;
    myListener.OnGame_GageBoss_Swap(index);
    myListener.OnGame_GageBoss_Focus(index);
};
myListener.OnGame_GageBoss_Remove = function(index)
{
    var _loc1_ = container_mc.gageBox["gageMc" + index];
    ...
    if(index == 1)
    {
        _loc2_ = container_mc.gageBox.gageMc0;
        myListener.OnGame_GageBoss_Swap(0);
        myListener.OnGame_GageBoss_Focus(0);
    }
    else
    {
        _loc2_ = container_mc.gageBox.gageMc1;
        myListener.OnGame_GageBoss_Swap(1);
        myListener.OnGame_GageBoss_Focus(1);
    }
};
myListener.OnGame_GageBoss_Focus = function(index)
{
    var _loc1_ = container_mc.gageBox["gageMc" + index];
    if(!gageFocus)
    {
...
myListener.OnGame_GageBoss_Swap = function(index)
{
...
myListener.OnGame_GageBoss_SetColorRotation = function(_colorList)
{
    ColorArray = [];
};
myListener.OnGame_GageBoss_SetBossData = function(bossName, questType)
{
    if(gageFocus == gageTop)
    {
...
myListener.OnGame_GageBoss_PercentVisible = function(bool)
{
    container_mc.percentTxt._visible = Boolean(Number(bool));
    textUpData();
};
myListener.OnGame_GageBoss_SetHp = function(curHP, maxHP, maxRank)
{
    ...
    GageChange(_loc1_);
    var _loc3_ = Math.round(curHP % _loc2_ / _loc2_ * gageWidth);
...
myListener.OnGame_GageBoss_UpDataHp = function(curHP, critical)
{
    var _loc1_ = gageFocus;
    var _loc2_ = 0;
    var _loc5_ = _loc1_.totalGage;
    var _loc6_ = _loc1_.cuttingGage;
    var _loc7_ = _loc1_.reSize;
    _loc1_.percentGage = int(curHP / _loc5_ * 100);
    textUpData();
    if(_loc5_ < curHP)
    {
        _loc2_ = _loc5_;
    ...
    var _loc8_ = 0;
    var _loc4_ = int(_loc1_.currentGage._width / _loc7_ + (_loc1_.prevRank - 1) * _loc6_);
    gs.TweenLite.killTweensOf(_loc1_.effectBar,false);
    gs.TweenLite.killTweensOf(_loc1_.effectGage,false);
    gs.TweenLite.killTweensOf(_loc1_.currentGage,false);
    if(_loc4_ < curHP)
    {
        GageUpMotion(_loc1_,_loc2_ - _loc4_);
    }
    else if(_loc4_ > curHP)
    {
        GageDownMotion(_loc1_,_loc4_ - _loc2_);
    }
};
myListener.OnGame_GageBoss_DangerMode = function(bool)
{
...
myListener.OnGame_GageBoss_SetSt = function(curST, maxST, stackMin, switchs)
{
    ...
    if(switchs == 1)
    {
        ...
        gs.TweenLite.to(container_mc.stackGage,0.2,{_alpha:30,onComplete:StackMotionEnd});
    }
};
...
myListener.OnGame_GageBoss_SetPattern = function(idx, type)
{
...
debuff_array.cnt = 0;
var cnt = 0;
myListener.OnGame_GageBoss_AddBuff = function(id, stackCnt, type, bRelaxMerit, img, remain, total)
{
    cnt++;
    type = Number(type);
        ...
        if(bRelaxMerit != 1)
        {
            ...
            else
            {
                ...
                if(remain <= 0)
                {
                    maked.cool.remove();
                    delete maked.cool;
                    myListener.OnGame_GageBoss_RemoveBuff(id);
                }
                else
                {
...
myListener.OnGame_GageBoss_UpDateBuff = function(id, stackCnt, useRelaxMerit, img, remain, total)
{
...
myListener.OnGame_GageBoss_RemoveBuff = function(id)
{
...
myListener.OnGame_GageBoss_ClearBuff = function(type)
{
    ...
    if(isNaN(type))
    {
        ...
        {
            myListener.OnGame_GageBoss_RemoveBuff(_loc2_[_loc1_].id);
            _loc1_ = _loc1_ + 1;
        }
        _loc1_ = 0;
        while(_loc1_ < _loc3_.length)
        {
            myListener.OnGame_GageBoss_RemoveBuff(_loc3_[_loc1_].id);
            _loc1_ = _loc1_ + 1;
        }
        return undefined;
    }
    ...
    if(type == 0)
    {
        ...
        {
            myListener.OnGame_GageBoss_RemoveBuff(imsi_arrray[_loc1_].id);
            _loc1_ = _loc1_ + 1;
        }
    }
    else if(type >= 1)
...
myListener.OnGame_GageBoss_HoldBuff = function(id)
{
...
myListener.OnGame_GageBoss_Add(0);
myListener.OnGame_GageBoss_Focus(0);
ToGame_GageBoss_Init();
myListener.OnGame_AutoTest = function(num)
{
    ...
    container_mc.onEnterFrame = function()
    {
        count--;
        gage = gage - num;
        myListener.OnGame_GageBoss_UpDataHp(gage);
        if(count < 0)
        {
...
myListener.OnGame_AutoTest2 = function()
{
    ...
    container_mc.onEnterFrame = function()
    {
        ...
        if(timer == 0)
        {
            ...
            myListener.OnGame_GageBoss_UpDataHp(gage,cevent);
            if(gage <= 0)
            {
                delete this.onEnterFrame;
            }
...
myListener.OnGame_AutoTest3 = function()
{
    ...
    myListener.OnGame_GageBoss_UpDataHp(testGage,_loc2_);
};

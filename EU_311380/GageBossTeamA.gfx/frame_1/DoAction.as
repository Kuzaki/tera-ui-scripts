...
function ToGame_GageBossTeamA_Init()
{
    fscommand("ToGame_GageBossTeamA_Init");
}
function ToGame_GageBossTeamA_PercentVisible(type)
{
    getURL("FSCommand:ToGame_GageBossTeamA_PercentVisible",type);
}
function GageUpMotionEnd(targetMc, num)
{
    lib.Debuger.trace("모션" + targetMc.prevRank);
    targetMc.prevRank++;
    textUpData();
    GageChange(targetMc);
...
function GageUpMotion(targetMc, num)
{
    ...
    {
        _loc2_ = gageWidth * (_loc3_ / _loc5_ + num) / cuttingGage;
...
container_mc.UIDrager.onRelease2 = function()
{
    if(container_mc.UIDrager.pos_state == "false")
    {
        ...
        {
            myListener.OnGame_GageBossTeamA_PercentVisible(0);
            ToGame_GageBossTeamA_PercentVisible(0);
        }
        else
        {
            myListener.OnGame_GageBossTeamA_PercentVisible(1);
            ToGame_GageBossTeamA_PercentVisible(1);
        }
    }
};
...
myListener.OnGame_GageBossTeamA_Add = function(index)
{
    var _loc4_ = 10 - index;
    var _loc1_ = container_mc.gageBox["gageMc" + index];
    if(!_loc1_)
    {
        _loc1_ = container_mc.gageBox.attachMovie("gageMc","gageMc" + index,_loc4_);
    }
    var _loc3_ = index * 20;
    _loc1_._x = _loc3_;
    _loc1_._y = - _loc3_;
...
myListener.OnGame_GageBossTeamA_Focus = function(index)
{
    var _loc1_ = container_mc.gageBox["gageMc" + index];
    if(!gageFocus)
    {
...
myListener.OnGame_GageBossTeamA_Swap = function(index)
{
...
myListener.OnGame_GageBossTeamA_SetColorRotation = function(_colorList)
{
    ColorArray = [];
};
myListener.OnGame_GageBossTeamA_SetBossData = function(bossName, questType)
{
    if(gageFocus == gageTop)
    {
...
myListener.OnGame_GageBossTeamA_PercentVisible = function(bool)
{
    container_mc.percentTxt._visible = Boolean(Number(bool));
    textUpData();
};
myListener.OnGame_GageBossTeamA_SetHp = function(curHP, maxHP, maxRank)
{
    ...
    var _loc4_ = maxHP;
    var _loc2_ = _loc4_ / maxRank;
    var _loc6_ = gageWidth / _loc2_;
    _loc1_.totalGage = _loc4_;
    _loc1_.cuttingGage = _loc2_;
    _loc1_.reSize = _loc6_;
    _loc1_.prevRank = int(curHP / _loc2_) + 1;
...
myListener.OnGame_GageBossTeamA_UpDataHp = function(curHP, critical)
{
...
myListener.OnGame_GageBossTeamA_DangerMode = function(bool)
{
...
myListener.OnGame_GageBossTeamA_SetSt = function(curST, maxST, stackMin, switchs)
{
...
myListener.OnGame_GageBossTeamA_SetPattern = function(idx, type)
{
...
debuff_array.cnt = 0;
var cnt = 0;
myListener.OnGame_GageBossTeamA_AddBuff = function(id, stackCnt, type, bRelaxMerit, img, remain, total)
{
    cnt++;
    type = Number(type);
        ...
        if(bRelaxMerit != 1)
        {
            ...
            else
            {
                ...
                if(remain <= 0)
                {
                    maked.cool.remove();
                    delete maked.cool;
                    myListener.OnGame_GageBossTeamA_RemoveBuff(id);
                }
                else
                {
...
myListener.OnGame_GageBossTeamA_UpDateBuff = function(id, stackCnt, useRelaxMerit, img, remain, total)
{
...
myListener.OnGame_GageBossTeamA_RemoveBuff = function(id)
{
...
myListener.OnGame_GageBossTeamA_ClearBuff = function(type)
{
    ...
    if(isNaN(type))
    {
        ...
        {
            myListener.OnGame_GageBossTeamA_RemoveBuff(_loc2_[_loc1_].id);
            _loc1_ = _loc1_ + 1;
        }
        _loc1_ = 0;
        while(_loc1_ < _loc3_.length)
        {
            myListener.OnGame_GageBossTeamA_RemoveBuff(_loc3_[_loc1_].id);
            _loc1_ = _loc1_ + 1;
        }
        return undefined;
    }
    ...
    if(type == 0)
    {
        ...
        {
            myListener.OnGame_GageBossTeamA_RemoveBuff(imsi_arrray[_loc1_].id);
            _loc1_ = _loc1_ + 1;
        }
    }
    else if(type >= 1)
...
myListener.OnGame_GageBossTeamA_HoldBuff = function(id)
{
...
myListener.OnGame_GageBossTeamA_TeamName = function(teamName)
{
    container_mc.teamName.htmlText = teamName;
};
var sidx = 1000;
...
myListener.OnGame_GageBossTeamA_Add(0);
myListener.OnGame_GageBossTeamA_Focus(0);
ToGame_GageBossTeamA_Init();
myListener.OnGame_AutoTest = function(num)
{
    ...
    container_mc.onEnterFrame = function()
    {
        count--;
        gage = gage - num;
        myListener.OnGame_GageBossTeamA_UpDataHp(gage);
        if(count < 0)
        {
            delete this.onEnterFrame;
        }
...
myListener.OnGame_AutoTest2 = function()
{
    ...
    container_mc.onEnterFrame = function()
    {
        ...
        if(timer == 0)
        {
            ...
            myListener.OnGame_GageBossTeamA_UpDataHp(gage,cevent);
            if(gage <= 0)
            {
                delete this.onEnterFrame;
            }
...
myListener.OnGame_AutoTest3 = function()
{
    ...
    myListener.OnGame_GageBossTeamA_UpDataHp(testGage,_loc2_);
};

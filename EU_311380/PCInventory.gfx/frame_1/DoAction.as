...
myListener.OnGame_PCInventory_SetSlot = function(index, img, count, _remainTime, _totalTime, disable, active, grade)
{
    ...
    var _loc5_ = container_mc.slotLayer;
    var _loc7_ = slotData[index].pX;
    var _loc6_ = slotData[index].pY;
    var _loc10_ = _loc5_["PCInventory__" + index];
    var _loc11_ = container_mc["amountFd__" + index];
    if(count != -1)
    {
        _loc11_.text = count;
    }
    ...
    if(slotData[index].bActive != active)
    {
        ...
        if(active)
        {
            var _loc8_ = _loc5_.attachMovie("FX_ACTIVE","FX_ACTIVE" + index,_loc5_.getNextHighestDepth());
            _loc8_._x = _loc7_ - 4;
            _loc8_._y = _loc6_ - 6;
        ...
        {
            _loc5_["FX_ACTIVE" + index].removeMovieClip();
        }
    }
    if(slotData[index].remain != _remainTime || _remainTime == -1)
    {
        ...
        if(_loc2_ == undefined)
        {
            ...
            _loc2_.onFinished = function()
            {
                ...
                _loc2_._x = this._x;
                _loc2_._y = this._y;
                lib.util.DrawBitmap.makeMcAndDraw(_loc2_.a_mc,img,50,50);
                lib.util.DrawBitmap.makeMcAndDraw(_loc2_.b_mc,img,50,50);
                lib.util.DrawBitmap.makeMcAndDraw(_loc2_.c_mc,img,50,50);
            };
        }
        if(_remainTime <= 0 || _totalTime <= 0)
        {
            if(_loc2_.coolTime != undefined)
            {
                ...
                var _loc3_ = container_mc.attachMovie("effect_standby","effect_standby" + index,container_mc.getNextHighestDepth());
                _loc3_.hitTestDisable = false;
                _loc3_._x = _loc2_._x;
                _loc3_._y = _loc2_._y;
                lib.util.DrawBitmap.makeMcAndDraw(_loc3_.a_mc,img,60,60);
                lib.util.DrawBitmap.makeMcAndDraw(_loc3_.b_mc,img,60,60);
                lib.util.DrawBitmap.makeMcAndDraw(_loc3_.c_mc,img,60,60);
            }
        }
        else
        {
    ...
    if(img != undefined && img != "" && img != slotData[index].img)
    {
        ...
        lib.manager.ToolTip.add(_loc10_,index,6);
    }

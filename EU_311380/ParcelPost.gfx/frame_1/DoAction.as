...
function setMessage(__name, _title, _message, date, attachItems, gold, recieve, isSystem, messageType, goldType)
{
    ...
    {
        _title = "<font color=\'#" + lib.info.TextColor.SYSTEM_MESSAGE.toString(16) + "\'>" + SYSTEM_STR + "</font>" + _title;
    }
...
function clearMessage()
{
    ...
    btn0_btn.setVisible(false);
    btn2_btn.txt.text = OK_STR;
    btn2_btn.setRelease(ToGame_ParcelPost_CloseUI);
    container_mc.goldLb.text = ATTGOLD_STR;
    input_btn.setEnabled(false);
    container_mc.recieveStateMc._alpha = 0;
}
function changeSkin(_mode, messageType, recieveState, sendTypeType)
{
    ...
    if(_mode == 0)
    {
        ...
        container_mc.comissionMc._alpha = 0;
        selectCheckBtn(messageType,false);
        container_mc.attchContainerMc.goldMc._visible = recieveState != 1 && sendTypeType != 1;
        if(messageType == 0)
        {
            container_mc.noItemFd._visible = true;
            container_mc.attchContainerMc._alpha = 0;
            btn0_btn.txt.text = RESEND_STR;
            btn0_btn.setVisible(true);
        ...
        {
            container_mc.attchContainerMc._alpha = 100;
            btn0_btn.txt.text = RESEND_STR;
            btn0_btn.setVisible(true);
            btn0_btn.setRelease(ToGame_ParcelPost_ReSend);
            btn2_btn.txt.text = RECIEVE_STR;
            btn2_btn.setRelease(ToGame_ParcelPost_Receive);
            container_mc.attchContainerMc.goldLb.text = ATTGOLD_STR;
            if(sendTypeType == 1)
            {
                btn0_btn.setVisible(false);
        ...
        else
        {
            ...
            container_mc.attchContainerMc.goldLb.text = ASTROGOLD_STR;
            btn0_btn.setVisible(recieveState == 0);
            btn0_btn.txt.text = SENDBACK_STR;
            ...
            if(sendTypeType == 1)
            {
                ...
                btn2_btn.txt.text = OK_STR;
                btn2_btn.setRelease(ToGame_ParcelPost_CloseUI);
                container_mc.recieveStateMc._alpha = 100;
            }
            goldLogTag = ASTROGOLD_STR;
        }
        if(recieveState == 1)
    ...
    else
    {
        ...
        selectCheckBtn(0,true);
        container_mc.comissionMc._alpha = 100;
        btn0_btn.setVisible(true);
...
myListener.OnGame_ParcelPost_EscrowMessage = function(__name, _title, _message, date, attachItems, gold, recieve, isSystem, messageType, goldType)
{
    clearMessage();
    if(goldType == undefined || goldType == "" || goldType == "undefined")
    {
    ...
    changeSkin(0,2,Number(recieve),Number(messageType));
    setMessage(__name,_title,_message,date,attachItems,gold,recieve,isSystem,messageType,goldType);
...
{
    container_mc.nameFd.searchResult(lib.util.ExtString.split(_results,"\t"));
};

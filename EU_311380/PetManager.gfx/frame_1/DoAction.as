...
while(i < 4)
{
    var slotMc = container_mc.skillContainer["PetManager__" + i];
    slotMc.draw("init");
    slotMc.index = i;
    slotMc.coolTimeMask._visible = false;
    slotMc.FX_DISABLE._visible = false;
    slotMc.Slot.disableSlot = true;
    slotMc.SLOT.enabled = false;
    slotMc.remain = slotMc.total = 0;
    slotMc.FX_ACTIVE.gotoAndStop(1);
    i++;
}
...
myListener.OnGameEventShowWindow = function(_widgetName, _bShow)
{
    if(_widgetName.toLowerCase() == UIname.toLowerCase())
    {
        ...
        else
        {
            ...
            while(_loc3_ < 4)
            {
                var _loc2_ = container_mc.skillContainer["PetManager__" + _loc3_];
...
myListener.OnGame_PetManager_SetUpdateSlot = function(_SlotIndex, _bActive, _SkillId, _IconName, _SkillName, _remain, _total)
{
    ...
    var _loc7_ = DATA_NAME + _SlotIndex;
    var _loc2_ = container_mc.skillContainer[_loc7_];
...
myListener.OnGame_PetManager_SetToggle = function(_SkillId, _bTrue)
{
    ...
    var _loc4_ = searchSkillInSlot(_SkillId);
    var _loc1_ = 0;
    while(_loc1_ < _loc4_.length)
    {
        var _loc2_ = DATA_NAME + _loc4_[_loc1_];
        var _loc3_ = container_mc.skillContainer[_loc2_];
        _loc3_.Slot.active = _bTrue;
        _loc1_ = _loc1_ + 1;
    }
};
...
myListener.OnGameEventSetSkillEnable = function(_SkillId, _bEnable)
{
    ...
    {
        var _loc3_ = DATA_NAME + _loc5_[_loc2_];
        var _loc1_ = container_mc.skillContainer[_loc3_].Slot;
        _loc1_.Icon._visible = true;
        _loc1_.SLOT.enabled = bEnable;
        _loc1_.bgs._visible = true;
        if(_bEnable)
        {
            var _loc4_ = new flash.geom.Transform(_loc1_.Icon);
            _loc4_.colorTransform = enableTrans;
            _loc1_.coolTimeMask.gotoAndStop(420);
        }
        else
        {
            _loc4_ = new flash.geom.Transform(_loc1_.Icon);
            _loc4_.colorTransform = disableTrans;
...
myListener.OnGameEventSetSkillCoolTime = function(_SkillId, _remain, _total)
{
    ...
    while(_loc2_ < 4)
    {
        var _loc3_ = DATA_NAME + _loc2_;
        if(arrSkill[_loc2_] == _SkillId)
        {
            var mc = container_mc.skillContainer[_loc3_].Slot;

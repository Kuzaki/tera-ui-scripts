...
{
    lib.Debuger.trace("ToGame_SelectCharacter_ListChange : " + id + "/" + num);
...
{
    lib.Debuger.trace("ToGame_SelectCharacter_BannerClick:" + s_Id);
...
myListener.OnGame_SelectCharacter_InviteCode = function(bMode, txt0, txt1, btnMode)
{
    ...
    if(titleMc.eventBox._visible)
    {
        titleMc.bannerBox._y = 340;
    }
    else
    {
        titleMc.bannerBox._y = 220;
    }
...
{
    var _loc3_ = lib.util.ExtString.split(s_DataList,"\t");
    var _loc4_ = _loc3_[0];
    var _loc6_ = _loc3_[1];
    var _loc7_ = Number(_loc3_[2]) + 1;
    var _loc5_ = _loc3_[3];
    var _loc2_ = listMc.box["line" + _loc4_];
    if(!_loc2_)
    {
        _loc2_ = listMc.box.attachMovie("lineMc","line" + _loc4_,n_ChangeDepth);
        n_ChangeDepth++;
        n_LineCount++;
        ...
        _loc2_.classMc.gotoAndStop(_loc7_);
        _loc2_.txt.htmlText = lib.util.UIString.getUIString("$012046");
        _loc2_.txt.htmlText = _loc2_.txt.htmlText + (_loc5_ + " " + _loc6_);
    }
...
myListener.OnGame_SelectCharacter_SetDetailData = function(s_DataList)
{
    ...
    centerMc.infoBox.txt1.htmlText = _loc1_[0];
...
{
    lib.Debuger.trace("마우스 업" + b_DragMode);

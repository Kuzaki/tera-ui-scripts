...
function setSkill(info)
{
    ...
    var _loc1_ = _loc8_[slotPrefix + _loc3_];
    if(_loc1_ == undefined)
    {
        _loc1_ = createSlot(_loc8_,_loc3_);
    }
    else
    {
        clearSkill(_loc1_);
    ...
    var _loc12_ = info[SKILL_NAME];
    var _loc6_ = info[SKILL_IMG];
    var _loc11_ = flash.display.BitmapData.loadBitmap(_loc6_);
    var _loc9_ = Number(info[SKILL_DISABLE]);
    var _loc5_ = Number(info[SKILL_STATE]);
    var _loc4_ = Number(info[SKILL_NUM]);
    var _loc13_ = info[SKILL_REMAIN];
    var _loc10_ = info[SKILL_TOTAL];
    myListener.OnGame_SkillWindow_CoolTime(_loc3_,_loc13_,_loc10_);
    var _loc15_ = _loc1_.slotHit._x;
    var _loc14_ = _loc1_.slotHit._y;
    _loc1_.slotIndex = _loc3_;
    _loc1_.skillId = _loc7_;
    _loc1_.nameFd.htmlText = _loc12_;
    _loc1_.disable = _loc9_;
    _loc1_.skillState = _loc5_;
    _loc1_.TEX = _loc6_;
    lib.display.DrawBitmapData.draw(_loc1_.img,_loc11_,0,0,0,0,38,38,0.59);
...
function setSkillState(slotIndex)
{
    ...
    var _loc3_ = _loc4_[slotPrefix + slotIndex];
    var _loc1_ = _loc3_.skillState;
    var _loc2_ = _loc4_["stateMc_" + slotIndex];
    if(_loc2_ != undefined)
    {
        _loc2_.removeMovieClip();
    }
    ...
    {
        _loc1_ = _loc1_ <= 1?1:2;
        _loc2_ = _loc4_.attachMovie("slotStateMc_" + _loc1_,"stateMc_" + slotIndex,_loc4_.getNextHighestDepth());
        _loc2_._x = _loc1_ <= 1?_loc3_._x + 1.5:_loc3_._x + 1;
        _loc2_._y = _loc1_ <= 1?_loc3_._y + 1:_loc3_._y + 1;
...
myListener.OnGame_SkillWindow_SetSkill = function(skillData, selectedSlot)
{
    ...
    var _loc3_ = [];
    lib.Debuger.trace("OnGame_SkillWindow_SetSkill********************************");
    var _loc2_ = 0;
    while(_loc2_ < _loc7_)
    {

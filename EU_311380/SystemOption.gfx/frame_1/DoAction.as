...
function ToGame_SystemOption_ResetScale()
{
    ...
    while(_loc2_ < scaleBFUIs.length)
    {
        ...
        _loc4_ = 100;
        if(_loc3_ == lib.gamedata.CustomizedUIDataKey.UISCALE_MAINMENU_PERCENT)
        {
            _loc4_ = 80;
        }
        custormizedData.saveData(_loc3_,_loc4_,true);
        _loc2_ = _loc2_ + 1;
    }
    ...
    for(var _loc2_ in keyMapOptionCon)
    {
        if(typeof keyMapOptionCon[_loc2_] == "movieclip")
        {
            if(keyMapOptionCon[_loc2_].code0 == _keycodes && keyMapOptionCon[_loc2_].key0_txt != selectKeyBox)
            {
                ...
                overapKey.textColor = _loc0_ = lib.info.TextColor.GENERAL_CRITICAL;
                selectKeyBox.textColor = _loc0_;
                return undefined;
            }
...
function getUICustormizeData()
{
    ...
    contMc.CWPB_mc.checked = Boolean(Number(custormizedData.valueOf(lib.gamedata.CustomizedUIDataKey.CHARACTERWINDOW_PERCENT_BOOLEAN)));
    contMc.CWPB_mc.onChanged = function(checked)
    {
        custormizedData.saveData(lib.gamedata.CustomizedUIDataKey.CHARACTERWINDOW_PERCENT_BOOLEAN,Number(checked));
    };
    ...
    var _loc13_ = {};
    _loc13_.onChanged = function(target, id, index)
    {
    ...
    contMc.BUSC_mc0.combo_mc.addListener(_loc13_);
    contMc.BUSC_mc0.combo_mc.selectedIndex = 0;
    contMc.battleModeSubTitleMc.BVUI_mc.onChecked = function(checked)
    {
        var _loc5_ = !checked?lib.info.TextColor.CHECKBTN_DISABLE:lib.info.TextColor.CHECKBTN_NORMAL;
        var _loc1_ = 0;
        while(_loc1_ < battleModeVisibleKey.length)
        {
            var _loc3_ = battleModeVisibleKey[_loc1_];
            var _loc2_ = container_mc.setUp4_mc[_loc3_ + "_mc"];
            _loc2_.setEnabled(checked);
            _loc2_.txt.textColor = _loc5_;
            _loc1_ = _loc1_ + 1;
        }
        custormizedData.saveData(lib.gamedata.CustomizedUIDataKey.BATTLEMODE_VISIBLE_UI_BOOLEAN,Number(checked));
    };
    contMc.battleModeSubTitleMc.BVUI_mc.checked = Boolean(Number(custormizedData.valueOf(lib.gamedata.CustomizedUIDataKey.BATTLEMODE_VISIBLE_UI_BOOLEAN)));
    var _loc9_ = 0;
...
container_mc.setUp2_mc.videoOPFd0.textAutoSize = "shrink";
container_mc.setUp2_mc.videoOPFd0.verticalAlign = "center";
...
var gameOptionIndex = ["SN","PN","ON","GN","HN","MN","VN","CI","CS","US","SC","SB","SR","UA","SQ","QN","QM","HA","RI","RT","RB","RF","RP","RG","RD","RW","SP","UM","TN","USC","QMK","QMM0","QMM1","QMM2","QMM3","NCA","CCA","HT","HP","HBE","HBB","ASS","HMV","PMV","MME","MSI","MDB","MMAR","NMAR","MMA","MDM","MDA","PTY","PSI","PDB","PDM","PDR","PDA","PHR","CH","SWAO","CMF","CMH","MG","CMD","BMSB","BMMT","BMRB"];
...
var videoOptionIndex = ["SR","RN","FS","CR","CL","GR","SF","SFS","AS","AF","CD","SD","BD","MD","WD","CE","BE","KD","SP","VL","FD","TS","HM","LL","MU","LS","ES","PE","TX","GEL","BEQ","BEL","LOD","FQ","SL"];
...
lib.manager.Focus.addFocusGroup(UIname,6,[container_mc.setUp2_mc.GEL_mc.valueMc.txt,container_mc.setUp2_mc.BEQ_mc.valueMc.txt,container_mc.setUp2_mc.BEL_mc.valueMc.txt,container_mc.setUp2_mc.LOD_mc.valueMc.txt]);
...
var detailQus = ["CD","SD","BD","MD","WD","CE","BE","KD","SP","VL","FD","TS","HM","ES","PE","TT","GEL","BEQ","BEL","LOD","FQ","CR","TX"];
var comboLostner = {};
...
lib.manager.ToolTip.add(container_mc.setUp2_mc.LOD_mc,"$231330",1);
...
var battleModeTooltip = lib.util.UIString.getUIString("$231327");
lib.manager.ToolTip.add(container_mc.setUp4_mc.battleModeTitleMc,battleModeTooltip,1);
lib.manager.ToolTip.add(container_mc.setUp4_mc.battleModeSubTitleMc,battleModeTooltip,1);
...
myListener.OnGame_SystemOption_InputKey = function(pKeyCode)
{
    ...
    if(pKeyCode == 13)
    {
        var _loc2_ = keyCodes.split("+");
        var _loc3_ = true;
        var _loc1_ = _loc2_.length - 1;
        while(_loc1_ >= 0 && _loc3_)
        {
            _loc3_ = _loc2_[_loc1_] == 17 || _loc2_[_loc1_] == 16 || _loc2_[_loc1_] == 18;
            _loc1_ = _loc1_ - 1;
        }
        if(_loc3_)
        {
...
var battleModeVisibleKey = ["BVPW","BVQU","BVMN","BVPI","BVMM","BVCT","BVOV"];
var i = 0;
while(i < battleModeVisibleKey.length)
{
    var targetKey = battleModeVisibleKey[i];
    var tooltipTarget = container_mc.setUp4_mc[targetKey + "_mc"];
    lib.manager.ToolTip.add(tooltipTarget,"$231328",1);
    i++;
}
getUICustormizeData();

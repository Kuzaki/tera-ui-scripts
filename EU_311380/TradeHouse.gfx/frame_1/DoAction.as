...
function selectedSortBtnOfWaitList(index, bNotifyClient)
{
    ...
    var _loc1_ = index;
    var _loc3_ = 0;
    lib.Debuger.trace("waitSlotObj.pressCount : " + waitSlotObj.pressCount + ", bNotifyClient : " + bNotifyClient);
    waitSlotObj.pressCount = !(waitSlotObj.pressCount == undefined || waitSlotObj.index != _loc1_)?waitSlotObj.pressCount + 1:0;
    lib.Debuger.trace("waitSlotObj.pressCount : " + waitSlotObj.pressCount);
...
var DefaultMinMax = [];
myListener.OnGame_TradeHouse_DetailSearchOption = function(itemType, sealType, itemLevelMIn, itemLevelMax, extractLevelMin, extractLevelMax, enchantLevelMin, enchantLevelMax, priceMin, priceMax, tcatMin, tcatMax, bDefault)
{
    if(Number(bDefault) == 1)
    {
        DefaultMinMax[2] = {min:Number(itemLevelMIn),max:Number(itemLevelMax)};
        DefaultMinMax[0] = {min:Number(extractLevelMin),max:Number(extractLevelMax)};
        DefaultMinMax[1] = {min:Number(enchantLevelMin),max:Number(enchantLevelMax)};
        DefaultMinMax[3] = {min:Number(priceMin),max:Number(priceMax)};
        DefaultMinMax[4] = {min:Number(tcatMin),max:Number(tcatMax)};
        resetDetailSearchOption();
        return undefined;
    }
    resetDetailSearchOption();
    var _loc3_ = container_mc.itemList_mc.detailOptionMc;
    var _loc2_ = lib.util.ExtString.split(itemType,"");
    var _loc1_ = 0;
    while(_loc1_ < _loc2_.length)
    {
        _loc3_["checkBox1_" + (_loc1_ + 1)].checked = _loc2_[_loc1_] == "1";
        _loc1_ = _loc1_ + 1;
    }
    _loc3_.radioBox0_0.checked = false;
    _loc3_["radioBox0_" + sealType].checked = true;
    if(Number(extractLevelMin) != DefaultMinMax[0].min)
    {
        detailMintxt0.text = extractLevelMin;
    }
    if(Number(extractLevelMax) != DefaultMinMax[0].max)
    {
        detailMaxtxt0.text = extractLevelMax;
    }
    if(Number(enchantLevelMin) != DefaultMinMax[1].min)
    {
        detailMintxt1.text = enchantLevelMin;
    }
    if(Number(enchantLevelMax) != DefaultMinMax[1].max)
    {
        detailMaxtxt1.text = enchantLevelMax;
    }
    if(Number(itemLevelMIn) != DefaultMinMax[2].min)
    {
        detailMintxt2.text = itemLevelMIn;
    }
    if(Number(itemLevelMax) != DefaultMinMax[2].max)
    {
        detailMaxtxt2.text = itemLevelMax;
    }
    detailMintxt3.total = priceMin;
    detailMaxtxt3.total = priceMax;
    detailMintxt4.text = tcatMin;
    detailMaxtxt4.text = tcatMax;
    checkDetailOption();
};
var goldType = 0;
...
openSmartOption();
container_mc.wait_mc.combobox.data = [{id:0,label:"$321119"},{id:1,label:"$321120"},{id:3,label:"$321122"},{id:6,label:"$321125"},{id:7,label:"$321126"},{id:8,label:"$321127"},{id:9,label:"$321128"},{id:10,label:"$321129"}];
container_mc.sold_mc.combobox.data = [{id:0,label:"$321119"},{id:1,label:"$321120"},{id:3,label:"$321122"},{id:6,label:"$321125"},{id:7,label:"$321126"},{id:8,label:"$321127"},{id:9,label:"$321128"},{id:10,label:"$321129"}];
var sortListner = {};
...
container_mc.sold_mc.sortBtn.onSelected = function(index)
{
    ...
    if(_loc3_ == 4)
    {
        if(soldSlotObj.pressCount % 4 < 2)
        {
            ...
            ToGame_TradeHouse_SortSoldList(4,soldSlotObj.pressCount % 2 + 1);
    ...
    else if(_loc3_ == 2)
    {
        ...
        ToGame_TradeHouse_SortSoldList(2,soldSlotObj.pressCount % 2 + 1);
    }
    else if(_loc3_ == 3)
    {
        _loc4_ = soldSlotObj.pressCount % 2 + 1;

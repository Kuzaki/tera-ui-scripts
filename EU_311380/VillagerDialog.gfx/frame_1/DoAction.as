...
myListener.OnGame_VillagerDialog_CreateApplyBtn = function(bTrue, applyBtnStr)
{
    ...
    if(bTrue)
    {
        ...
        if(_loc6_ != -1)
        {
            _loc9_ = _loc10_.attachMovie("keyMc","keyMc",_loc10_.getNextHighestDepth());
            _loc9_._y = 9;
            var _loc7_ = getKeyStr(applyBtnStr.substring(_loc6_ + 1));
            lib.Debuger.trace("keyStr : " + _loc7_);
            applyBtnStr = applyBtnStr.substring(0,_loc6_);
            _loc10_.txt.text = applyBtnStr;
            var _loc3_ = _loc10_.txt.textWidth + 5;
            _loc9_.bg.hitTestDisable = true;
            _loc9_.textFd.text = _loc7_;
            var _loc1_ = Math.max(_loc9_.textFd.textWidth + 5,14);
            lib.Debuger.trace("keyMc.textFd.textWidth + 5 : " + _loc9_.textFd.textWidth + 5 + ", keyTextW : " + _loc1_);
            ...
            if(_loc2_ > _loc8_)
            {
                ...
                _loc3_ = _loc4_ <= 80?_loc8_ - _loc4_:347;
            }
            _loc10_.txt._width = _loc3_;
            _loc2_ = Math.min(_loc2_,427);
            _loc10_.txt._x = 10 + (427 - _loc2_) / 2;
            _loc9_.textFd._width = _loc1_;
            _loc9_.textFd.setText(_loc7_);
            _loc9_.bg._width = _loc1_ + 6;
            _loc9_._x = _loc10_.txt._x + _loc3_;
        }
...
myListener.OnGame_VillagerDialog_SetDialog = function(dialogText, questName, dialogBtnData, bBaseRequitalComplete, baseRequitalItemData, repayMoney, bRepeatRequitalComplete, repeatRequitalCompleteNum, repeatRequitalItemData, bSelectableRequitalComplete, selectableRequitalCompleteNum, selectableRequitalItemData, bReduceRepeatQuestReward, questType, bAddRequitalComplete, AddRequitalItemData, rewardTitles, pointInfo)
{
    ...
    if(_loc16_.length >= 1)
    {
        ...
        while(_loc4_ < _loc16_.length)
        {
            ...
            var _loc6_ = _loc3_.indexOf("#");
            lib.Debuger.trace("sIndex : " + _loc6_);
            if(_loc6_ != -1)
            {
                var _loc14_ = getKeyStr(_loc3_.substring(_loc6_ + 1));
                _loc3_ = _loc3_.substring(0,_loc6_);
                btn.txt.text = _loc3_;
                var _loc7_ = btn.txt.textWidth + 5;
                ...
                var _loc8_ = 20 + _loc24_;
                var _loc21_ = _loc8_ + _loc7_;
                if(_loc21_ > 385)
                {
                    _loc5_ = _loc8_ <= 80?_loc5_:74;
                    _loc7_ = _loc8_ <= 80?385 - _loc8_:305;
                }
                btn.txt._width = _loc7_;
                _loc2_.textFd._width = _loc5_;
                _loc2_.textFd.setText(_loc14_);
                _loc2_.bg._width = _loc5_ + 6;
                _loc2_._x = btn._x + btn.txt._x + _loc7_;
            }

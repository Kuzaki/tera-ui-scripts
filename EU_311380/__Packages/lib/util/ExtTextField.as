class lib.util.ExtTextField
{
    ...
    var exceptComma = false;
    var _useFocus = true;
    var useAciveColor = true;
    function ExtTextField(ptextField, type, focusMC)
    {
        ...
        switch(this._type)
        {
            ...
            case 1:
                this._inerlistner.onChanged = function(textFd)
                {
                    if(_this.exceptComma)
                    {
                        _this.onChangedNumber(textFd,1);
                    }
                    else
                    {
    ...
    function onChangedPaste(textFd)
    {
        switch(this._type)
        {
            case 0:
                this.onChangedGenal(textFd);
                break;
            case 1:
                if(this.exceptComma)
                {
                    this.onChangedNumber(textFd,true);
                }
                else
                {
                    this.onChangedNumber(textFd);
                }
                break;
            case 2:
                this.onChangedEn(textFd);
                break;
            case 3:
                this.onChangedKr(textFd);
        }
    }
    ...
    {
        if(this.useAciveColor)
        {
            this.__get__textField().textColor = lib.info.TextColor.INPUT_ACTIVE;
        }
        if(this._type == 1 && !this.exceptComma)
        {
            this._textField.text = lib.util.CurrencyFormat.makeComma(str);
            if(str == "")
            {
                this.noexceptComma_Num = 0;
            }
            else
            {
                this.noexceptComma_Num = str;
            }
        }
        else
        {
    ...
    function __get__text()
    {
        if(this._type == 1 && !this.exceptComma)
        {
            return lib.util.ExtString.trimChar(this._textField.text);
        }
    ...
    function onChangedNumber(textFd, exceptComma)
    {
        this.clearInputInterval(this._intervals);
        var _loc4_ = false;
        var _loc5_ = textFd.text;
        var _loc2_ = String(Number(lib.util.ExtString.trimChar(textFd.text)));
        if(isNaN(_loc2_))
        {
            _loc4_ = true;
        }
        if(exceptComma && _loc5_.split(".").length == 2)
        {
            var _loc6_ = textFd.text.length;
            var _loc7_ = _loc5_.substr(0,_loc6_ - 1);
            var _loc8_ = _loc5_.substr(_loc6_ - 1,1);
            if(!isNaN(Number(_loc7_)) && _loc8_ == ".")
            {
                _loc2_ = textFd.text;
                _loc4_ = false;
            }
        ...
        {
            _loc2_ = "";
            this.__set__text(_loc2_);
            this.checkInterval(String(this.__get__min()));
        }
        ...
        {
            this.__set__text(_loc2_);
            Selection.setSelection(this.__get__textField().text.length,this.__get__textField().text.length);
        }
        _loc2_ = textFd.text;
